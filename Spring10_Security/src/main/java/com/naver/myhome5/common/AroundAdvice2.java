...
//Advice : 횡단 관심에 해당하는 공토 기능의 의미하며 독립된 클래스의 메서드로 작성됩니다.
...
public class AroundAdvice2 {
    ...
    *   즉, 클라이언트의 요청을 가로챈 어드바이스는 클라이언트가 호출한 비즈니스 메서드 (ServiceImpl 의 get으로 시작하는 메서드) 를
   ...
   public Object aroundLog(ProceedingJoinPoint proceeding) throws Throwable{
      ...
      // 대상 객체의 메서드 (예-BoardServiceOImpl.getListCount()) 를 호출합니다.
      Object result = proceeding.proceed();
      
      sw.stop();
      logger.info("=========================================================================");
      logger.info("[Around Advice2의 after]: 비즈니스 메서드 수행 후 입니다.");
      
      //호출되는 메서드에 대한 정보를 구합니다.
      Signature sig = proceeding.getSignature();
      
      /*
       * org.aspectj.lang.Signature 인터페이스를 호출되는 메서드와 관련된 정보를 제공합니다.
