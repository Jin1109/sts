...
public class BoardController {
   ...
   //@RequestMapping(value=" /write",method+RequestMethd.GET)
    ...
    * 매개변수의 이름과 setter와 property가 일치하면 됩니다.
    */
   @PostMapping("/add")
           ...
           //어느 주소에서 detail로 이동했는지 header의 정보 중에서 "referer"를 통해 알 수 있습니다.
   {
      ...
      if(beforeURL.endsWith("list")) { //myhome5/board/member/list 에서 제목을 클릭한 경우 조회수 증가하도록 합니다.
   ...
   public String BoardModifyAction(
      ...
      if (check != null && !check.equals("")) { // 기존파일 그대로 사용하는 경우입니다.
         ...
         //위 문장 떄문에 board.setBOARD_FILE()값은 자동 저장됩니다.
      } else {
         ...
         if (uploadfile!=null && !uploadfile.isEmpty()) {
            ...
            boarddata.setBOARD_FILE("");//""로 초기화 됩니다.
   ...
   public ModelAndView BoardReplyView(int num, ModelAndView mv,
	   ...
	   if (board == null) {
		   mv.setViewName("error/error");
		   mv.addObject("url", request.getRequestURL());
		   mv.addObject("message", "게사판 답변글 가져오기 실패");
	   }else {
		   mv.addObject("boarddata", board);
   ...
   public ModelAndView BoardReplyAcion(Board board, ModelAndView mv,
		   							   HttpServletRequest request,
		   							   RedirectAttributes rattr
		    ) {
	   	...
	   	if (result == 0) {
	   		mv.setViewName("error/error");
	   		mv.addObject("url", request.getRequestURL());
	   		mv.addObject("message", "게사판 답변 처리실패");
	   	}else {
   ...
   public String BoardDeleteAction(String BOARD_PASS, int num, 
	   ...
	   // 글 삭제 명렬을 요청한 사용자가 글을작성한 사용자인지 판단하기 위해
	   ...
	   // 삭제 처리 실패한 경우
	   if (result == 0) {
		   logger.info("게사판 삭제 실패");
		   mv.addAttribute("url", request.getRequestURL());
		   mv.addAttribute("message", "삭제 실패");
		   return "error/error";
	   }
	   
	   // 삭제 철리 성공한 경우 - 글 몰록 보기 요청을 전송하는 부분입니다.
   ...
   public byte[] BoardFileDown(String filename, 
      ...
      //Context-Disposition: attachment: 브라우저는 해당 Content를 처리하지 않고, 다운로드하게 됩니다.
